...
int Node::GetLoad(string c)
{
    ...
    _c_addr.sin_addr.s_addr = inet_addr("127.0.0.1");
    _c_addr.sin_port = htons(p2);

    char msg[BUFLEN] = {0};
    int len = sizeof("Fuck");
    snprintf(msg, len, "Fuck");
    n = sendto(_node_socket_fd, msg, len, 0, (struct sockaddr *)&_c_addr, _slen);
    // cout<<n<<endl;
}