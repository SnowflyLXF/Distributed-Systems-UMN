...
int Node::handleDownload(string dip, int dport)
{
    thread uplder(&Node::Uploader, this, dip, dport);
    uplder.detach();
    num_up ++;

}

void Node::Uploader(string dip, int dport)
{
    load++;
    ...
    uploader_addr.sin_port = htons(p);

    uploader_socket_fd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);

    if (::bind(uploader_socket_fd, (struct sockaddr *)&uploader_addr, sizeof(uploader_addr)) == -1)
    {
        cout << "Bind failed!" << endl;
    }
