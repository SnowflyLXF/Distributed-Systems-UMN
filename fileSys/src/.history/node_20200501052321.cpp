...
Node ::Node(int Port, int OnPort, int hbPort)
{
    num_up = 0;
    load = 0;
    _port = Port;
    ...
    tv0.tv_sec = 3;
    tv0.tv_usec = 0;
    if (setsockopt(_node_socket_fd, SOL_SOCKET, SO_RCVTIMEO, &tv0, sizeof(tv0)) < 0)
    {
    ...
    thread handlehb(&Node::handleheartbeat, this);
    handlehb.detach();
}

int Node::handleheartbeat()
{
    memset((char *)&_hb_addr, 0, sizeof(_hb_addr));
    _hb_addr.sin_family = AF_INET;
    _hb_addr.sin_addr.s_addr = htonl(INADDR_ANY);
    _hb_addr.sin_port = htons(_hbPort);

    _hb_socket_fd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    struct timeval tvhb;
    tvhb.tv_sec = 3;
    tvhb.tv_usec = 0;
    if (setsockopt(_hb_socket_fd, SOL_SOCKET, SO_RCVTIMEO, &tvhb, sizeof(tvhb)) < 0)
    {
        printf("time out setting failed\n");
    }

    if (::bind(_hb_socket_fd, (struct sockaddr *)&_hb_addr, sizeof(_node_addr)) == -1)
    {
        cout << "Bind failed!" << endl;
    }
    ...
    int res = 0;
    while(1){
        memset(_bufhb, ' ', BUFLEN);
        int recvLen = recvfrom(_hb_socket_fd, _bufhb, BUFLEN, 0, (struct sockaddr *)&_tmp_addr, reinterpret_cast<socklen_t *>(&_slen));
        if (recvLen == -1 && errno == EAGAIN)
        {
            printf("Server timeout, maybe down now, wait for recovery\n");
            res = 1;
            Update();
            sleep(3);
        }else
...
{
    char msg[BUFLEN] = {0};
    string packet = "FileList:" + to_string(_onListenPort) + ":" + to_string(_hbPort);

    names files = getFileList();
    int fsize = files.size();

    packet = packet + ":" + to_string(fsize);

    for (int i = 0; i < fsize; i++)
    {
        packet = packet + ":" + files[i];
    }
    int len = packet.length() + 1;
    snprintf(msg, len, packet.c_str());
    int n = sendall(msg, &len);
}
...
{
    char msg[BUFLEN] = {0};
    string packet = "Update:"+ to_string(_onListenPort) + ":" +to_string(_hbPort); 
...
int Node::GetLoad(string c)
{
    ...
    char _bufd[BUFLEN];
    int recvLen = recvfrom(_node_socket_fd, _bufd, BUFLEN, 0, (struct sockaddr *)&_tmp_addr, reinterpret_cast<socklen_t *>(&_slen));
    if (recvLen == -1 && errno == EAGAIN)
    {
        printf("timeout, try another peer\n");
        return -1;
    }
    return atoi(_bufd);
}
...
int Node::Download(string file)
{
    ...
    thread onDownload(&Node::handleDownload, this, file);
...
int Node::downloadfrom(string uper, string file)
{
    ...
    tv.tv_sec = 3;
    tv.tv_usec = 0;
    if (setsockopt(down_node_socket_fd, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv)) < 0)
    {
        printf("time out setting failed\n");
    }
...
int Node::handleDownload(string file){
    ...
    string uper = nodelist.front().second;
    int ret = downloadfrom(uper, file);
